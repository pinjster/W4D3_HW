# DESCRIPTION:
# You are stacking some boxes containing gold weights on top of each other. If a box contains more weight than the box below it, it will crash downwards and combine their weights. 
...
# [2]     [5]              [2]     [3]
# [1] --> [1] --> [6], not [1] --> [3] 
# Weights will always be integers. The matrix (array of arrays) may have any height or width >= 1, and may not be square, 
#but it will always be "nice" (all rows will have the same number of columns, etc).

#intaking a list of lists
#check second list, then following lists
#if num in index in list 2 is smaller than index in list 1 then add list 1 index value to list 2 index value 
#return bottom list 
